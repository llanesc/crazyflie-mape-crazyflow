"""MAPPO with MPC Context Management for LEAP-C Policies.

This is a minimal modification of SKRL's MAPPO to support MPC warmstart context
passing, similar to how PPO_RNN handles hidden states.

This file is for reference only - it shows the pattern for context management.

Based on: https://github.com/Toni-SM/skrl/blob/develop/skrl/multi_agents/torch/mappo/mappo.py
"""
...
MAPPO_LEAPC_CTX_DEFAULT_CONFIG = {
...
class MAPPOLeapCCtx(MultiAgent):
    """MAPPO with MPC context management for LEAP-C policies.

    This is a minimal modification of SKRL's MAPPO. Changes are marked with
    '# === MPC CONTEXT ===' comments.
    """
    ...
    def __init__(
        ...
        """Multi-Agent Proximal Policy Optimization (MAPPO) with MPC Context
        ...
        """
        _cfg = copy.deepcopy(MAPPO_LEAPC_CTX_DEFAULT_CONFIG)
        ...
        # === MPC CONTEXT: Initialize per-agent context storage (like RNN hidden states) ===
        self._mpc_initial_ctx: dict[str, Any] = {uid: None for uid in self.possible_agents}
        self._mpc_final_ctx: dict[str, Any] = {uid: None for uid in self.possible_agents}
        # ===================================================================================
    ...
    def act(self, states: Mapping[str, torch.Tensor], timestep: int, timesteps: int) -> torch.Tensor:
        ...
        with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
            data = []
            for uid in self.possible_agents:
                # === MPC CONTEXT: Build inputs with context ===
                inputs = {"states": self._state_preprocessor[uid](states[uid])}
                inputs["mpc_ctx"] = self._mpc_initial_ctx[uid]
                # ==============================================

                action, log_prob, outputs = self.policies[uid].act(inputs, role="policy")
                data.append((action, log_prob, outputs))

                # === MPC CONTEXT: Capture returned context ===
                if "mpc_ctx" in outputs:
                    self._mpc_final_ctx[uid] = outputs["mpc_ctx"]
                # ==============================================
    ...
    def record_transition(
        ...
        if self.memories:
            ...
            for uid in self.possible_agents:
                ...
                # === MPC CONTEXT: Flow context and reset for done episodes ===
                self._mpc_initial_ctx[uid] = self._mpc_final_ctx[uid]

                # Reset context for terminated/truncated episodes
                done = terminated[uid] | truncated[uid]
                if done.any() and self._mpc_initial_ctx[uid] is not None:
                    self._reset_mpc_ctx_for_done(uid, done, next_states[uid])
                # =============================================================

    # === MPC CONTEXT: Helper to reset context for done episodes ===
    def _reset_mpc_ctx_for_done(
        self,
        uid: str,
        done_mask: torch.Tensor,
        next_states: torch.Tensor,
    ) -> None:
        """Reset MPC context for environments that terminated/truncated.

        Similar to how PPO_RNN zeros out hidden states for finished episodes.

        :param uid: Agent UID
        :param done_mask: Boolean tensor indicating which envs are done
        :param next_states: Next states (for initializing reset context)
        """
        ctx = self._mpc_initial_ctx[uid]
        if ctx is None or not hasattr(ctx, "iterate"):
            return

        done_idx = done_mask.squeeze().cpu().numpy().astype(bool)
        if not done_idx.any():
            return

        # Get policy's MPC layer for physical parameters
        policy = self.policies[uid]
        if not hasattr(policy, "mpc_layer"):
            return

        mpc_layer = policy.mpc_layer
        N = mpc_layer.mpc_horizon
        nx = 12  # State dimension
        nu = 4   # Control dimension

        # Reset state trajectory to new initial state
        new_x0 = next_states[done_idx].detach().cpu().numpy()
        new_x_tiled = np.tile(new_x0, (1, N + 1))
        ctx.iterate.x[done_idx, :] = new_x_tiled

        # Reset control trajectory to hover
        hover_thrust = mpc_layer.mass * mpc_layer.gravity
        hover_u = np.array([0.0, 0.0, 0.0, hover_thrust])
        n_done = done_idx.sum()
        hover_u_tiled = np.tile(hover_u, (n_done, N))
        ctx.iterate.u[done_idx, :] = hover_u_tiled

        # Reset dual variables
        ctx.iterate.pi[done_idx] = 0.0
        ctx.iterate.lam[done_idx] = 0.0
        if ctx.iterate.sl.size > 0:
            ctx.iterate.sl[done_idx] = 0.0
        if ctx.iterate.su.size > 0:
            ctx.iterate.su[done_idx] = 0.0
    # ===============================================================

    # === MPC CONTEXT: Public method to reset all contexts ===
    def reset_mpc_context(self, uid: Optional[str] = None) -> None:
        """Reset MPC warmstart context.

        :param uid: If provided, only reset context for this agent.
                    If None, reset context for all agents.
        """
        if uid is not None:
            self._mpc_initial_ctx[uid] = None
            self._mpc_final_ctx[uid] = None
        else:
            for uid in self.possible_agents:
                self._mpc_initial_ctx[uid] = None
                self._mpc_final_ctx[uid] = None
