#!/usr/bin/env python3
"""Evaluation script for trained MAPPO ACMPC models on Red vs Blue environment.

This script evaluates trained blue evader policies (ACMPC) against scripted red pursuers.
"""
...
def load_configs(checkpoint_path: Path) -> tuple[dict, dict | None]:
    ...
    raise FileNotFoundError(
        ...
        "Make sure the checkpoint is in an experiment directory created by train_mappo_acmpc.py."
    )
...
def evaluate(env, policy, n_episodes, deterministic=False, render=False, render_fps=30, record_path=None,
    ...
    # Track per-world step counts and rewards (for accurate multi-world tracking)
